...
template <typename T>
void partialSumWithLock(long long &s, T begin, T end)
{
    long long l_sum = sum(begin, end);
    mtx.lock();
    s += l_sum;
    mtx.unlock();
}
...
long long parallelSumSingleThread(const std::vector<int> &vec)
{
    ...
    return sum(vec.cbegin(), vec.cend());
}
...
size_t parallelCountSingleThread(const std::vector<int> &vec, int target)
{
    ...
    return linearSearch(vec.cbegin(), vec.cend(), target);
}
...
std::vector<int> parallelFillSingleThread(size_t n)
{
    ...
    fillVector(vec.begin(), vec.end(), 0);
    return vec;
}
...
std::pair<int, size_t> parallelMaxElementLowestIndexSingleThread(const std::vector<int> &vec)
{
    ...
    return findMax(vec.cbegin(), vec.cend(), 0);
}
...
std::pair<int, size_t> parallelMaxElementLowestIndexTwoThreads(const std::vector<int> &vec)
{
    ...
    if (partialMaxes[0].first == partialMaxes[1].first)
    {
        return partialMaxes[0].second < partialMaxes[1].second ? partialMaxes[0] : partialMaxes[1];
    }
    return partialMaxes[0].first > partialMaxes[1].first ? partialMaxes[0] : partialMaxes[1];
...
void task5(const std::vector<int> &vec, size_t t)
{
    ...
    std::cout << "N Threads:" << std::endl;
    start = std::chrono::high_resolution_clock::now();
    display_pair(parallelMaxElementLowestIndexNThreads(vec, t));
    stop = std::chrono::high_resolution_clock::now();
    duration = duration_cast<std::chrono::microseconds>(stop - start);
    std::cout << "duration = " << duration << std::endl;
}